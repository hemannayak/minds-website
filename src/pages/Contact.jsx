...
const Field = ({ label, id, ...props }) => (
    <div className="space-y-1.5">
        <label htmlFor={id} className="text-xs font-semibold tracking-[0.08em] uppercase text-white/40 block">
            {label}
        </label>
        {props.as === 'textarea' ? (
            <textarea
                id={id}
                {...props}
                className="w-full bg-white/[0.04] border border-white/10 rounded-[10px] px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 focus:ring-1 focus:ring-white/20 transition-all duration-200 resize-none"
            />
        ) : (
            <input
                id={id}
                {...props}
                className="w-full bg-white/[0.04] border border-white/10 rounded-[10px] px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 focus:ring-1 focus:ring-white/20 transition-all duration-200"
            />
        )}
    </div>
);
                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        >
                            <motion.div variants={fadeInUp} className="max-w-2xl">
                                <p className="text-xs font-semibold tracking-[0.12em] uppercase text-white/40 mb-5">Contact</p>
                    ...
                    <div className="max-w-7xl mx-auto">
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
                                ...
                                {contactPoints.map((c) => {
                                    ...
                                    const inner = (
                                        <div className="flex items-start gap-4">
                                            <div className="w-9 h-9 rounded-[8px] bg-white/[0.06] border border-white/10 flex items-center justify-center shrink-0">
                                                <Icon size={16} className="text-white/70" />
                                            </div>
                                            <div className="min-w-0">
                                                <p className="text-[10px] font-bold tracking-[0.14em] uppercase text-white/40 mb-1">{c.label}</p>
                                                <p className="text-sm font-bold text-white leading-snug break-words">{c.value}</p>
                                                {c.sub && <p className="text-xs text-white/40 mt-0.5 leading-relaxed">{c.sub}</p>}
                                            </div>
                                        </div>
                                    );
                                    return (
                                        <motion.div key={c.label} variants={fadeInUp}>
                                            {c.href ? (
                                                <a href={c.href} className="block bg-white/[0.04] ring-1 ring-white/10 rounded-[14px] p-6 hover:-translate-y-[2px] transition-all duration-200 group" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.4),0 8px 24px rgba(0,0,0,0.3)' }}>
                                                    {inner}
                                                </a>
                                            ) : (
                                                <div className="bg-white/[0.04] ring-1 ring-white/10 rounded-[14px] p-6" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.4),0 8px 24px rgba(0,0,0,0.3)' }}>
                                                    {inner}
                                                </div>
                                            )}
                                        </motion.div>
                            ...
                            <motion.div
                                ...
                                className="lg:col-span-3 bg-white/[0.04] rounded-[14px] ring-1 ring-white/10 overflow-hidden"
                            ...
                            >
                                ...
                                <div className="px-8 py-6 border-b border-white/[0.06] flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white/40 text-xs">We'll reply within 24 hours</p>
                                    </div>
                                </div>
                                    ...
                                    {formStatus === 'success' ? (
                                        ...
                                        <form onSubmit={handleSubmit} className="space-y-5">
                                            ...
                                            <button
                                                ...
                                                className="w-full py-3.5 rounded-[10px] font-bold text-sm text-black bg-white hover:bg-white/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-[250ms] ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[1px] flex items-center justify-center gap-2"
                                            ...
                                            <p className="text-center text-xs text-white/40">
                                                ...
                                                <a href="mailto:minds.datascience@hitam.org" className="text-white/70 font-semibold hover:underline">
